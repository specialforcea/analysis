...
def raw_to_OD(fpath):
    with h5py.File(fpath) as h5_file:
        ...
        atoms1_mask = (MOT_Fluorescence_atoms1/100.)
        
        
        dark_mask = (MOT_Fluorescence_dark/100.)
        
        atoms2_mask = (MOT_Fluorescence_atoms2/100.)
    ...
    with h5py.File(path) as h5_file:
        ...
        # Compute number
            if True: #stored == "z-TOF":
                count_atoms1 = np.sum((atoms1_-dark_)[100:280,60:300])
        ...
        # OD and ROI display
            subplot(gs[0])
            im0= imshow(atoms1_[100:280,60:300], vmin= -0.0,  cmap='prism', aspect='auto', interpolation='none')
            ...
            subplot(gs[1])
            im0= imshow(dark_, vmin= -0.0,  cmap='flag', aspect='auto', interpolation='none')
            divider = make_axes_locatable(gca())
            cax = divider.append_axes("right", "5%", pad="3%")
            colorbar(im0, cax=cax)
            title('dark')
            ...
            subplot(gs[2])
            im0= imshow(atoms2_, vmin= -0.0,  cmap='RdYlBu_r', aspect='auto', interpolation='none')
            divider = make_axes_locatable(gca())
            cax = divider.append_axes("right", "5%", pad="3%")
            colorbar(im0, cax=cax)
            title('atoms2')
