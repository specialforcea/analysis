...
def get_zernike_guess(image):
    x0, y0 = mahotas.center_of_mass(image)
    a0, a1, a2, a3, a4, a5 = mahotas.features.zernike_moments(image, 
                                          radius=amax(image.shape)/2,degree = 3)
    return x0, y0, a0, a1, a2, a3, a4, a5
...
def zernike_2d(X, Y, x0, y0, a0, a1, a2, a3, a4, a5): 
    result = ne.evaluate('a0 + a1*(X-x0) + a2*(Y-y0) + a3*(2*sqrt((X-x0)**2+(Y-y0)**2)) + a4*((X-x0)**2-(Y-y0)**2) + a5*(X-x0)*(Y-y0)')
    return result
...
def fit_2d(image, fit_function='Gaussian', binsize=1, clip=0, mask=0, **kwargs):
    ...
    elif 'zernike' in fit_function.lower():
        fitfn = zernike_2d
        params_guess = get_zernike_guess(imagef)
    else:
        fitfn = tf_2d
        params_guess = get_gaussian_guess(imagef)
    ...
    if fitfn.__name__ == 'gaussian_2d':
        ...
        u_N_int = sqrt(sum((u_params[2:5]/params[2:5])**2)) * abs(N_int)
    elif fitfn.__name__ == 'zernike_2d':
        N_int = params[2:5].prod()
        u_N_int = sqrt(sum((u_params[2:5]/params[2:5])**2)) * abs(N_int)
    params = append(params, N_int)
    u_params = append(u_params, u_N_int)
    
    # prepare a dictionary of param_name : (param, u_param) pairs
    params_names_A = ['X0', 'Y0', 'XW', 'YW', 'Amp', 'Offset']
    params_names_B = ['X0', 'Y0', 'a0', 'a1', 'a2', 'a3', 'a4', 'a5']
    if fitfn.__name__ == 'gaussian_2d':
        params_names = ['Gaussian_' + p for p in params_names_A]
    elif fitfn.__name__ == 'tf_2d':
        params_names = ['ThomasFermi_' + p for p in params_names_A]
    elif fitfn.__name__ == 'zernike_2d':
        params_names = ['Zernike_' + p for p in params_names_B]    
    else:
        raise Exception('fit_function argument must be Gaussian/ThomasFermi/Zernike.')
