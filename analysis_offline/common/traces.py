...
# Quadratic function   
def quadratic(x, a, x0, off):
    return off + x0*x + a*x**2 #a*(x-x0)**2 + off
 
def fit_quadratic(xdata, ydata, u_ydata=None, **kwargs):
    return curve_fit(quadratic, xdata, ydata, [1.0, 1., 1.],u_ydata, **kwargs)

# Square Root
def square_root(x, x0, a, b):
    return a*sqrt(1+x/x0) + b
...
def lorentzian(x, amplitude, x0, fwhm, offset):
    return amplitude/(1 + (x-x0)**2/(fwhm)**2) + offset
...
def bimodal(x, gaussamp, gauss_center, gauss_width, gauss_offset, tf_amp, tf_center, tf_radius, tf_offset):
    return (gaussian(x, gaussamp, gauss_center, gauss_width, gauss_offset) +  
            thomas_fermi(x, tf_amp, tf_center, tf_radius, tf_offset))

def fit_bimodal(xdata, ydata, u_ydata=None, **kwargs):
    ymin =np.amin(ydata)
    ymax =np.amax(ydata)
    g_a = ymax - ymin
    #check for an inverted peak
    if np.abs(np.mean(ydata[[0,-1]]) - ymin) > np.abs(np.mean(ydata[[0,-1]]) - ymax):
        ...
        g_o = ymax
        g_a = -g_a
    g_c = np.sum(xdata*prob)/np.sum(prob)
    g_w = np.sum(np.abs(xdata-g_c)*prob)/np.sum(prob)
    tf_a = 3*g_a
    tf_c, tf_o, tf_w = g_c, g_o, g_w
    return curve_fit(bimodal, xdata, ydata, [g_a, 100.0, g_w, 1.0, tf_a, 100.0, tf_w, 1.0], u_ydata, **kwargs)
...
def gaussian(x, amplitude, x0, sigma, offset):
    x = np.array(x)
    return amplitude*exp(-(x-x0)**2/(2*sigma**2)) + 0*offset
