...
def fit_quadratic(xdata, ydata, u_ydata=None, **kwargs):
    xdata, ydata = np.array(xdata), np.array(ydata)
    print("It is a go!")
    return curve_fit(quadratic, xdata, ydata, [1e4, 1.0, 1e6],u_ydata, **kwargs)
...
def parabola(x, a, b):
    x = np.array(x)
    return sqrt(a*x**2 + b**2)
  
def fit_parabola(xdata, ydata, u_ydata=None, **kwargs):
    xdata, ydata = np.array(xdata), np.array(ydata)
    
    return curve_fit(parabola, xdata, ydata, [1e4, 100.0],u_ydata, **kwargs)
