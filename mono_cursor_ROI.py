...
class cursors_ROI(object):
    def __init__(self, z, x = None, y = None, n_ROI=1):
        ...
        self.shot.pcolormesh(self.z, cmap = 'viridis', vmin=-0.2, vmax=np.amax(self.z))
...
def MOT_raw_to_OD(fpath):
    with h5py.File(fpath) as h5_file:
        ...
        atoms1_mask = np.ma.masked_less_equal(MOT_Fluorescence_atoms1/np.max(MOT_Fluorescence_atoms1), 0.)
        
        
        dark_mask = np.ma.masked_less_equal(MOT_Fluorescence_dark/np.max(MOT_Fluorescence_dark), 0.)
        
        atoms2_mask = np.ma.masked_less_equal(MOT_Fluorescence_atoms2/np.max(MOT_Fluorescence_atoms2), 0.)
        
        atoms1 = np.matrix(atoms1_mask)
        dark = np.matrix(dark_mask)
        atoms2 = np.matrix(atoms2_mask)
                
                
        return atoms1, dark, atoms2

_OD_ = MOT_raw_to_OD(path)
